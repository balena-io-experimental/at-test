...
static uint16_t tftp_block = 1;
static int tftp_sock = -1;
extern int tftp_send_data(int sock, void *tx_buf, uint16_t block, long size);
extern int tftp_write_request(const char *serv_ip, const char *filename, long tsize, long blksize);
...
static bool sahara_start(void) {
    ...
    while (1)
    {
        ...
        case SAHARA_WAIT_MEMORY_REGION:
            ...
            for(i = 0; i < num_debug_entries; i++) {
                ...
                if (!strncmp(full_filename, "/dev/null", strlen("/dev/null"))) {
                    full_filename[strlen("/dev/null")] = '\0';
                }                   

                if (tftp_sock > 0) {
                    fd = tftp_write_request(kickstart_options.path_to_save_files+strlen(TFTP_F),
                        sahara_memory_table[i].filename, sahara_memory_table[i].length, sahara_data.max_ram_dump_read);
                    tftp_block = 1;
                }
                else 
                ...
                while (cur < sahara_memory_table[i].length) {
                    ...
                    if (tftp_sock > 0) {
                        retval = tftp_send_data(fd, sahara_data.rx_buffer, tftp_block++, len);
                        if (cur == sahara_memory_table[i].length && len == sahara_data.max_ram_dump_read) {
                            tftp_send_data(fd, NULL, tftp_block++, 0);
                        }
                    }
                    else
 ...
 int sahara_catch_dump(int port_fd, const char *path_to_save_files, int do_reset) {
    ...
    if (!strncmp(path_to_save_files, TFTP_F, strlen(TFTP_F))) {
        sahara_data.max_ram_dump_read = 16*1024;
        tftp_sock = tftp_write_request(kickstart_options.path_to_save_files+strlen(TFTP_F), "qlog_tftp_test_blksize",
            sahara_data.max_ram_dump_read - 1, sahara_data.max_ram_dump_read);
        if (tftp_sock > 0) {
            retval = tftp_send_data(tftp_sock, sahara_data.rx_buffer, 1, sahara_data.max_ram_dump_read-1);
            if (retval != sahara_data.max_ram_dump_read-1) {
                close(tftp_sock);
                tftp_sock = -1;
            }
        }
