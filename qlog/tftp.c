...
int tftp_write_request(const char *serv_ip, const char *filename, long tsize, long blksize) {
    ...
    charp = (char *)&tx_pkt->data[-2]; //tx_pkt->filename to avoid [-Wformat-overflow=]
    size = sprintf(charp, "%s%c%s%c%s%c%ld%c%s%c%ld%c", filename, 0, "octet", 0, "blksize", 0, blksize, 0, "tsize", 0, tsize, 0);	
