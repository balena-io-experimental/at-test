...
ssize_t mdm_send_cmd(int fd, const unsigned char *buf, size_t size) {
    size_t wc = 0;
...
static int mdm_init_filter(int fd, const char *cfg) {
    ...
    if (cfg && !strcmp(cfg, "dump")) {
        ...
        mdm_send_cmd(fd, mdm_enter_dump1, sizeof(mdm_enter_dump1));
        usleep(100*1000);
        mdm_send_cmd(fd, mdm_enter_dump2, sizeof(mdm_enter_dump2));
        return 0;
    }
#endif

    if (g_mdm_use_cfg2 && cfg) {
        mdm_init_filter(fd, NULL); //first send Default.cfg
    }
...
typedef struct _qshrink4_header {
...
static int mdm_logfile_init(int logfd, unsigned logfile_seq) {
    struct _qshrink4_header qmdlV2Header;
    size_t size = sizeof(struct _qshrink4_header);
    ssize_t nbytes;
    ...
    qmdlV2Header.header_length = qlog_le32(sizeof(qmdlV2Header));
    qmdlV2Header.version = 1;
    qmdlV2Header.hdlc_data_type = 1;
    qmdlV2Header.guid_list_entry_count = qlog_le32(0);

    // Write the file header.
    nbytes = write(logfd, &qmdlV2Header, size);
    if (nbytes != size) {
